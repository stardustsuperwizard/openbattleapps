...
<main class="container-md mt-5">
    <section class="row mt-3">
        <div class="col">
            <h1>Combat Phase - Close Combat</h1>
        </div>
    </section>
    <section class="row">
        <div class="col">
            <h2>Instructions</h2>
                <div v-if="!doneVariable">
                    <p><b>Player {{ currentPlayer }}</b>, may now select squads to make close combat attacks.</p>
                </div>
                ...
                <div v-else>
                    <PreviousMove class="btn btn-secondary mt-3 mx-1" v-bind:playerList="playerList" v-bind:indexNumber="index" v-model:index="index">Previous Move</PreviousMove>
                    <router-link to="morale" class="btn btn-primary mt-3 mx-1">Next Phase</router-link>
                </div>
        </div>
    </section>
    <section class="row mt-3" v-if="!doneVariable">
        <div class="col">
            <h2 class="border-bottom">
                <a href="#" class="text-dark text-decoration-none" v-on:click.prevent="(showCalculator = !showCalculator)" v-if="!showCalculator"> > Close Combat Calculator</a>
                <a href="#" class="text-dark text-decoration-none" v-on:click.prevent="(showCalculator = !showCalculator)" v-if="showCalculator"> v Close Combat Calculator</a>
            </h2>
        ...
        <div class="col">
            <h2 class="border-bottom">
                ...
                <a href="#" class="text-dark text-decoration-none" v-on:click.prevent="showRules = !showRules" v-if="showRules"> v Rules</a>
                    ...
                    <div v-if="showRules">
                        <BattleRules :tags="['close combat']"></BattleRules>
                    </div>
        </div>
    </section>
</main>
